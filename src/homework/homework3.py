...
def list_of_even_numbers(num):
    ...
    while number < num + 1:
        if number % 2 == 0:
            even_numbers = even_numbers + str(number) + ','
        number += 1
...
def main1():
    ...
    max_loop = int(input("Enter an integer: "))
    for index in range(1,max_loop+1):
        sum_odd_value = sum_odd_numbers(index)
        print(sum_odd_value)
        list_even_value = list_of_even_numbers(index)
        print(list_even_value)
